...
export async function POST(request: Request) {
  ...
  const pending = pendingSummaryWindows(segments, existing, MAX_WINDOWS_PER_CALL);
  ...
  for (const { windowIndex, sourceText } of pending) {
    ...
    // Save each finished window before starting the next paid request.
    const { error } = await supabase.from("lecture_summaries").upsert({
    ...
    }, { onConflict: "session_id,window_index", ignoreDuplicates: true });
    if (error) {
