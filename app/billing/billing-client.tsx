...
import { PLANS, STARTER_CREDITS, STARTER_DAYS, type PurchasePlan } from "../lib/plans";
...
export default function BillingClient({ locale, signedIn, mode, account }: { locale: "ko" | "en"; signedIn: boolean; mode: "sandbox" | "live" | "disabled"; account: Account }) {
  ...
  useEffect(() => {
    ...
    if (plan === "monthly" || plan === "semester") setSelected(plan);
  }, []);
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      <div className={styles.heading}><p>{t("수업에 맞는 만큼, 가볍게.", "A little more room for every lecture.")}</p><h1>{t("내 수업에 맞는 플랜", "Choose your pace.")}</h1><p>{t("기능은 모두 같아요. 필요한 시간과 결제 방식만 고르세요.", "Every feature is included. Choose the time and payment schedule that fit.")}</p></div>
      ...
      <div className={styles.plans}>
        {(Object.entries(PLANS) as [PurchasePlan, typeof PLANS[PurchasePlan]][]).map(([key, plan]) => <article key={key} className={`${styles.plan} ${key === "semester" ? styles.semester : ""} ${selected === key ? styles.selected : ""}`}>
          <div className={styles.planName}><h2>{plan.name}</h2><span>{plan.recurring ? t("매달 필요한 만큼", "Month by month") : t("한 학기를 여유롭게", "A semester at your pace")}</span></div>
          <p className={styles.price}>{en ? `$${plan.usd}` : `${plan.krw.toLocaleString()}원`}<span>{plan.recurring ? t("/ 월", "/ month") : t("/ 4개월", "/ 4 months")}</span></p>
          <div className={styles.allowance}><strong>{plan.credits.toLocaleString()} <span>credits</span></strong><p>{plan.recurring ? t(`매월 ${plan.credits / 60}시간의 강의 기록`, `${plan.credits / 60} recording hours each month`) : t(`${plan.credits / 60}시간의 강의 기록 · 한 번에 지급`, `${plan.credits / 60} recording hours · available upfront`)}</p></div>
          <p className={styles.renewal}>{plan.recurring ? t("매월 자동 결제 · 언제든 다음 갱신 해지", "Renews monthly · cancel the next renewal anytime") : t("한 번만 결제 · 자동 갱신 없음", "One payment · no automatic renewal")}</p>
          {!signedIn ? <Link className={styles.choose} href={`${base}/login?next=${encodeURIComponent(`${base}/billing?plan=${key}`)}`}>{t("로그인하고 선택", "Sign in to choose")}<ArrowRight size={18} /></Link> : <button className={styles.choose} onClick={() => { setSelected(key); void checkout.startCheckout(key); }} disabled={mode === "disabled" || !checkout.ready || Boolean(checkout.pending) || checkout.unconfirmed || (key === "monthly" && subscribed)}>{checkout.pending === key ? t("결제창 여는 중…", "Opening checkout…") : key === "monthly" && subscribed ? t("이용 중인 구독", "Current subscription") : mode === "disabled" ? t("준비 중", "Coming soon") : t(`${plan.name} 선택`, `Choose ${plan.name}`)}<ArrowRight size={18} /></button>}
          <p className={styles.expiry}>{plan.recurring ? t("각 결제 주기 말에 미사용 credits가 만료됩니다.", "Unused credits expire at the end of each billing period.") : t("결제일부터 4개월간 자유롭게 나눠 사용하세요.", "Use your credits across any classes within four months.")}</p>
        </article>)}
      </div>
      ...
      <section className={styles.included}><h2>{t("어떤 플랜이든, 수업의 처음부터 끝까지.", "Everything you need, in either plan.")}</h2><ul>{[t("한국어·영어·혼용 강의 기록", "Korean, English and mixed-language transcription"),t("강의 맥락을 읽는 질문과 답변", "Questions answered with your lecture’s context"),t("강의 자료 연결과 복습 노트", "Course materials and review notes")].map(item=><li key={item}><Check size={17}/>{item}</li>)}</ul></section>
      ...
      <section className={styles.faq} aria-label={t("결제 안내", "Billing questions")}>
        ...
        <details><summary>{t("추가로 구매하거나 구독을 해지할 수 있나요?", "Can I buy more or cancel?")}</summary><p>{t("Semester는 필요할 때 추가 구매할 수 있고, 각 구매분은 결제일로부터 4개월간 유효합니다. Monthly는 중복 구독할 수 없습니다. 결제·구독 관리에서 언제든 다음 갱신을 해지할 수 있으며 이미 결제한 credits는 원래 만료일까지 사용할 수 있어요.", "Buy another Semester whenever you need more; each purchase lasts four months. Only one Monthly subscription can be active. Cancel the next renewal in Manage billing; paid credits remain valid until their original expiry.")}</p></details>
