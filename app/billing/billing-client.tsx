  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      ...
      {signedIn && <div className={styles.account}>
        ...
        <div className={styles.accountActions}><button onClick={openTopup} aria-haspopup="dialog">{t("크레딧 추가", "Add credits")}</button>{account?.paddle_customer_id && <button onClick={openPortal} disabled={portalPending}>{portalPending ? t("여는 중…", "Opening…") : t("결제·구독 관리", "Manage billing")}</button>}</div>
      </div>}
      {!signedIn && selected === "topup" && <p className={styles.topupLogin}>{t("크레딧 추가는 로그인 후 이용할 수 있어요.", "Sign in to add credits.")} <Link href={languageSwitchUrl(`${base}/login?next=${encodeURIComponent(`${base}/billing?plan=topup`)}`, locale)} prefetch={false}>{t("로그인", "Sign in")}</Link></p>}
      ...
      <div className={styles.plans}>
        {CARD_PLANS.map((key) => {
          ...
          return <article key={key} className={`${styles.plan} ${key === "semester" ? styles.featured : ""} ${selected === key ? styles.selected : ""}`}>
            ...
            {!signedIn
              ? <Link className={styles.choose} href={languageSwitchUrl(`${base}/login?next=${encodeURIComponent(`${base}/billing?plan=${key}`)}`, locale)} prefetch={false}>{t("로그인하고 선택", "Sign in to choose")}</Link>
              : <button className={styles.choose} onClick={() => { setSelected(key); void checkout.startCheckout(key); }} disabled={!canChoose || !checkout.ready || Boolean(checkout.pending) || checkout.unconfirmed || blocked}>
                  {checkout.pending === key ? t("결제창 여는 중…", "Opening checkout…") : blocked ? activePlan === key || !activePlan && key === "monthly" ? t("이용 중인 플랜", "Current plan") : t("이용 기간 종료 후 선택", "Choose after this period") : !ready ? t("준비 중", "Coming soon") : priceState !== "ready" ? t("가격 확인 중", "Checking price") : t(`${plan.name} 선택`, `Choose ${plan.name}`)}
                </button>}
          </article>;
        })}
      </div>
      ...
      <section className={styles.trial}><div><h2>{t("먼저 한 수업에서 써보세요.", "Try it in your next class.")}</h2><p>{t(`새 계정으로 강의실을 처음 열면 ${STARTER_CREDITS} 크레딧을 무료로 받아요. 지급일부터 ${STARTER_DAYS}일 동안 사용하며, 카드 등록과 자동 결제는 없습니다.`, `Open your first classroom with a new account to receive ${STARTER_CREDITS} free credits, valid for ${STARTER_DAYS} days from issue. No card or automatic renewal.`)}</p></div><Link href={languageSwitchUrl(signedIn ? `${base}/classroom` : `${base}/login`, locale)} prefetch={false}>{t("무료로 시작", "Start free")}</Link></section>
    ...
    {signedIn && <dialog ref={topupDialog} className={styles.topupDialog} aria-labelledby="topup-title" aria-describedby="topup-description">
      ...
      <button type="button" className={styles.choose} disabled={!topupReady} onClick={() => { topupDialog.current?.close(); void checkout.startCheckout("topup"); }}>{checkout.pending === "topup" ? t("결제창 여는 중…", "Opening checkout…") : !topupAvailable ? t("준비 중", "Coming soon") : priceState !== "ready" ? t("가격 확인 중", "Checking price") : t("1,000 크레딧 구매", "Buy 1,000 credits")}</button>
