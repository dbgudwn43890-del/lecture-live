...
<section className="closing-section" aria-labelledby="closing-title"><div className="page-width closing-layout"><div><h2 id="closing-title">{t("놓친 부분을 묻고,")}<br />{" "}{t("다시 수업 속으로.")}</h2></div><div className="closing-actions"><a className="button button-light" href={startHref}>{t("무료로 내 강의 시작")}{" "}<span aria-hidden="true">{"→"}</span></a><button className="closing-demo" type="button" data-open-demo>{t("30초만 체험하기")}</button><span>{locale === "en" ? `${STARTER_CREDITS} free credits · ${STARTER_DAYS} days from issue · No card needed` : `${STARTER_CREDITS} 크레딧 무료 · 지급일부터 ${STARTER_DAYS}일 · 카드 등록 없이`}</span></div></div></section>
</main>
