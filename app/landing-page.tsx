...
<div className="hero-actions"><button className="button" type="button" data-open-demo aria-controls="demo-panel" aria-expanded="false">{t("직접 체험하기")}</button><a className="text-link" href={startHref} data-analytics-signup={isAuthenticated ? undefined : "true"}>{t("무료로 내 강의 시작")}</a></div>
...
<button className="excerpt-try" type="button" data-open-demo aria-controls="demo-panel" aria-expanded="false">{t("이 대목, 물어보기")}</button>
...
<button className="button mobile-explain" id="mobile-explain" type="button" aria-controls="demo-answer" aria-expanded="false">{t("왜 반대인지 물어보기")}</button>
...
<div className="demo-before" id="demo-before"><h3>{t("둘 다 B가 높은데,")}<br />{" "}{t("왜 합치면 반대예요?")}</h3><p>{t("앞에서 말한 ‘가중치’를 놓쳤어요.")}</p><button className="button" id="explain-button" type="button" aria-controls="demo-answer" aria-expanded="false">{t("이 질문 보내기")}</button><span className="fine-print">{t("샘플 답변 · 마이크 사용 없음")}</span></div>
...
<button className="source-link" id="source-button" type="button" aria-controls="earlier-context" aria-expanded="false">{t("37:05 · 앞에서 설명한 근거 보기")}</button>
<div className="what-if"><button id="equalize-button" type="button" aria-pressed="false">{t("PC·모바일을 반반씩 비교하면?")}</button><p id="what-if-result" hidden aria-live="polite">{t("같은 비중으로 비교하면 A는 50%, B는 57.5%예요. 달랐던 것은 각 페이지의 방문자 구성이었어요.")}</p></div>
</div>
...
<article className="journey-step"><span className="phase">{t("수업 전")}</span><h3>{t("시작하세요.")}</h3><p>{t("직접 시작을 눌러야 녹음해요. 강의 자료도 함께 올려두세요.")}</p><div className="start-snippet" aria-label={t("기록 시작 화면 예시")}><span>{t("오늘의 수업")}</span><strong>{t("통계학")}</strong><span className="snippet-rule"></span><span>{locale === "en" ? "English lecture" : t("한국어 강의")}</span><span className="snippet-action">{t("강의 시작")}</span></div></article>
...
<div className="review-copy"><h2 id="review-title">{t("내 질문에서")}<br />{" "}{t("시작하는 복습.")}</h2><p>{t("헷갈렸던 대목과 설명을")}<br />{" "}{t("복습 노트로 다시 꺼내보세요.")}</p><button className="text-link" id="preview-note-button" type="button" aria-controls="note-details" aria-expanded="false">{t("복습 노트 한 장 펼쳐보기")}</button></div>
...
<div><h2 id="faq-title">{t("궁금한 점이")}<br />{" "}{t("남았나요?")}</h2><a className="support-link" href="mailto:support@lecue.app">{"support@lecue.app"}</a></div>
...
<details><summary>{t("체험에는 가입이나 녹음이 필요한가요?")}<span aria-hidden="true">{"+"}</span></summary><p>{t("필요하지 않아요. 미리 구성한 통계 강의와 설명으로 Lecue의 흐름을 경험합니다. 마이크를 켜지 않으며, ‘체험 닫기’나 Escape 키로 바로 돌아올 수 있어요.")}</p><button className="text-link" type="button" data-open-demo>{t("지금 잠깐 체험하기")}</button></details>
...
<section className="closing-section" aria-labelledby="closing-title"><div className="page-width closing-layout"><div><h2 id="closing-title">{t("놓친 부분을 묻고,")}<br />{" "}{t("다시 수업 속으로.")}</h2></div><div className="closing-actions"><a className="button button-light" href={startHref} data-analytics-signup={isAuthenticated ? undefined : "true"}>{t("무료로 내 강의 시작")}</a><button className="closing-demo" type="button" data-open-demo>{t("직접 체험하기")}</button><span>{locale === "en" ? `${STARTER_CREDITS} free credits · ${STARTER_DAYS} days from issue · No card needed` : `${STARTER_CREDITS} 크레딧 무료 · 지급일부터 ${STARTER_DAYS}일 · 카드 등록 없이`}</span></div></div></section>
</main>
<footer className="site-footer page-width"><div className="footer-top"><a className="wordmark brand-lockup" href="#home" aria-label={t("Lecue 홈으로")}><span className="lecue-symbol" aria-hidden="true" />{"Lecue"}</a><p>{t("수업의 흐름을, 내 속도로.")}</p><a className="back-top" href="#home">{t("맨 위로")}</a></div><div className="footer-bottom"><span>{"© 2026 Lecue"}</span><nav aria-label={t("서비스 안내")}><a href={`${base}/privacy#cookies`} data-analytics-settings>{locale === "en" ? "Cookie settings" : "쿠키 설정"}</a><a href={`${base}/privacy`}>{t("개인정보처리방침")}</a><a href={`${base}/terms`}>{t("이용약관")}</a><a href="mailto:support@lecue.app">{t("문의")}</a><button id="theme-toggle" type="button">{t("화면 밝기")}</button></nav></div></footer>
