-- New purchases receive monthly installments. Existing orders/grants retain
-- their exact original quantities and immediate availability.
...
begin
  ...
  if p_plan='monthly' and exists(select 1 from public.billing_accounts where user_id=p_user_id
...
-- Replay effective issuance dates in chronological order. This is intentionally
-- safe to repeat, repairs late deliveries, and cannot cross an expiry gap.
-- Future installments do not renew or become spendable before starts_at.
create function public.refresh_credit_installments(p_user_id uuid) returns void
language plpgsql security definer set search_path='' as $$
declare v_event record; v_expiry timestamptz;
begin
  if p_user_id is null then return; end if;
  perform pg_advisory_xact_lock(hashtextextended('credit-renewal:'||p_user_id::text,0));
  for v_event in
    select distinct g.renewal_paid_at from public.credit_grants g
    where g.user_id=p_user_id and g.renewal_paid_at<=now() and g.revoked_at is null
      and g.granted_credits>g.refunded_credits
      and (g.purchase_transaction_id is not null or not exists(
        select 1 from public.billing_adjustments a where a.transaction_id=g.source_id and a.subtotal>0))
    order by g.renewal_paid_at
  loop
    v_expiry:=public.credit_utc_months(v_event.renewal_paid_at,12);
    update public.credit_grants g set expires_at=greatest(g.expires_at,v_expiry),updated_at=now()
    where g.user_id=p_user_id and g.remaining_credits>0 and g.revoked_at is null
      and g.starts_at<=v_event.renewal_paid_at and g.expires_at>v_event.renewal_paid_at
      and g.expires_at<v_expiry;
  end loop;
end $$;
revoke all on function public.refresh_credit_installments(uuid) from public,anon,authenticated;
...
begin
  ...
  if p_grant is not null then
    insert into public.credit_grants(user_id,source_type,source_id,plan_code,granted_credits,remaining_credits,starts_at,expires_at,paid_subtotal,
      purchase_transaction_id,installment_index,installment_count,renewal_paid_at)
    values(v_user_id,p_grant->>'source_type',p_grant->>'source_id',p_grant->>'plan_code',
      ...
      case when v_paid_at is not null then public.credit_utc_months(v_paid_at,12) else (p_grant->>'expires_at')::timestamptz end,
      ...
      case when v_paid_at is not null and (p_grant->>'paid_subtotal')::bigint>0 and (v_refund=0 or v_version='monthly_v1') then v_paid_at end)
    ...
    if v_inserted_id is not null and v_version='monthly_v1' and v_count>1 then
      for v_index in 1..v_count-1 loop
        ...
        insert into public.credit_grants(user_id,source_type,source_id,plan_code,granted_credits,remaining_credits,starts_at,expires_at,
          purchase_transaction_id,installment_index,installment_count,renewal_paid_at)
        values(v_user_id,'payment',v_tx||':installment:'||v_index,p_grant->>'plan_code',2400,2400,v_start,public.credit_utc_months(v_start,12),
          v_tx,v_index,v_count,v_start);
      end loop;
    end if;
  ...
  if v_tx is not null then
    ...
    if found and v_refund>0 then
      ...
      -- balances, latest installments first. If a later installment was already
      ...
      for v_pass in 1..2 loop
        for v_row in select * from public.credit_grants g where g.id=v_first.id or g.purchase_transaction_id=v_tx
          order by coalesce(g.installment_index,0) desc for update
        loop
  ...
  perform public.refresh_credit_installments(v_user_id);
...
-- This RPC now writes due renewal effects, so it must be VOLATILE. Its owner
-- identity still comes only from the authenticated JWT.
...
begin
  ...
  perform public.refresh_credit_installments(v_user);
  return query
    select coalesce(sum(g.remaining_credits) filter(where g.starts_at<=now() and g.expires_at>now() and g.revoked_at is null),0)::integer,
      ...
      (select p.plan_code from public.credit_grants p where p.user_id=v_user and p.installment_index=0
        and p.plan_code in ('monthly','semester','annual') and p.revoked_at is null and p.starts_at<=now()
        and public.credit_utc_months(p.starts_at,p.installment_count)>now()
        order by p.starts_at desc limit 1),
      (select public.credit_utc_months(p.starts_at,p.installment_count) from public.credit_grants p where p.user_id=v_user and p.installment_index=0
        and p.plan_code in ('monthly','semester','annual') and p.revoked_at is null and p.starts_at<=now()
        and public.credit_utc_months(p.starts_at,p.installment_count)>now()
        order by p.starts_at desc limit 1)
...
-- Both charge paths apply due issuance effects under the same owner lock.
...
as $$
begin
  perform public.refresh_credit_installments(auth.uid());
  return (select auth.uid()) is not null and (
    exists (
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('credit-renewal:'||new.user_id::text,0));
